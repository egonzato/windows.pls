...
#' @import dplyr
#' @import tidyr
#' @returns Plot of R2 of the region requested by the user.
#' @export
#' @examples
#' data(beer)
#' conc=beer[,1]
...
r2.single.window=function(wpls=NULL,
                               condition='Complete',
                               shape.cal=19,
                               shape.cv=19,
                               width=1,
  ...
  perf.r2.t=perf.r2.t[which(perf.r2.t$perf.rep==condition),]

  ggplot()+
    geom_point(data=perf.r2.t,aes(x=Component,y=R2, col='Cal', fill='Cal'),shape=shape.cal,size=size)+
    geom_point(data=perf.r2.t,aes(x=Component,y=perf.r2.cv.v, col='Cv', fill='Cv'),shape=shape.cv,size=size)+
    geom_path(data=perf.r2.t,aes(x=Component,y=R2, col='Cal'),linewidth=width)+
    geom_path(data=perf.r2.t,aes(x=Component,y=perf.r2.cv.v, col='Cv'),linewidth=width)+
